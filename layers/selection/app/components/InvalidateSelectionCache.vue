...
async function onConfirm() {
  ...
  try {
    const res = await $fetch<{ invalidationId?: string; status?: string }>(
      `/api/selections/${props.username}/${props.eventId}/invalidate-cache`,
      { method: 'POST' },
    )
    open.value = false
    toast.add({
...
<template>
  <UModal v-model:open="open" title="Invalidate Selection Cache">
    <template #body>
      <div class="space-y-4">
        <UAlert
          v-if="errorMessage"
        ...
        <UAlert
          ...
          :description="`Wildcard invalidation for '/${username}/${eventId}/selection/*'. Takes 5–15 min to propagate. Only needed when re-uploading photos at previously-served paths.`"
        />
      </div>
    </template>
