...
import type { UploadFile } from '../composables/useGalleryUpload'
...
const { files, isUploading, uploadError, totalBytes, uploadedBytes, overallProgress, addFiles, removeFile, reset, startUpload } =
  useGalleryUpload(props.username, props.eventId)
...
<template>
  <UModal v-model:open="open" title="Upload gallery images" :ui="{ content: 'max-w-2xl' }">
    <UButton
      label="Upload images"
